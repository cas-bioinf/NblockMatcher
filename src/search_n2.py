...
# reference implementation
def _n_compatible_2tuples(n_lists, dists):
    """Find compatible 2-tuples between given lists
    assumptions for tuple (a, b):
    - all tuples within a list must be same width that is b-a is constant for all tuples in a list
    - b > a for all tuples in a list
...
def _wrapper(n_lists, dists):
    ...
    return _n_compatible_2tuples(n_lists, dists)
...
if __name__ == "__main__":
    ...
    _res = _wrapper(test_lists, test_dists)
    print('sorted: ', sorted(_res) == sorted(ref))
    print('ref:', ref)
    print('res:', res)
